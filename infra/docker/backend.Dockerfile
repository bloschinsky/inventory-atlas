...
# Install only the backend runtime graph into a fresh stage. Never copy the
# development store: it contains generators, Prisma Studio and its EPL tooling.
FROM toolchain AS runtime-dependencies
COPY package.json pnpm-lock.yaml pnpm-workspace.yaml ./
COPY scripts/verify-toolchain.mjs scripts/verify-toolchain.mjs
COPY apps/api/package.json apps/api/package.json
COPY apps/worker/package.json apps/worker/package.json
...
COPY --from=runtime-dependencies --chown=node:node /workspace/apps/api/node_modules ./apps/api/node_modules
...
COPY --from=runtime-dependencies --chown=node:node /workspace/apps/worker/node_modules ./apps/worker/node_modules
